...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const getCartData = async () => {
      try {
        ...
        let total = json.reduce((sum, item) => sum + item.price, 0);
        setTotalPrice(total);
        setShipping(total < 100 && json.length > 0);
      } catch (error) {
        console.error("Error fetching products:", error.message);
      }
    };
  ...
  const changePrice = async (id, price) => {
    try {
      const response = await fetch(`${BASE_URL}/api/cart/changePrice`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "auth-token": token,
      ...
      if (json.success) {
        ...
        let total = json.data.reduce((sum, item) => sum + item.price, 0);
        setTotalPrice(total);
        setShipping(total < 100 && json.data.length > 0);
      }
    } catch (error) {
  ...
  const onCheckout = () => {
    const updatedData = data.map((item) => ({
      ...item,
      negotiatedPrice: newPrice[item.id] || item.price,
    }));
          ...
          <div className="row cart_body">
            <div className="col-md-8">
              <div className="d-flex align-items-center w-100">
                ...
                >
                  Clear Cart &nbsp;
                  <i className="fa-solid fa-trash delete-cart-all"></i>
                </button>
              </div>
                            ...
                            <tr key={item.id}>
                              ...
                              <td align="center">
                                <button
                                  ...
                                  onClick={() => onRemove(index)}
                                >
                                  <i className="fa-solid fa-trash delete_btn_cart"></i>
                                </button>
                              </td>
                            </tr>
                          );
